...
describe('.api()', function () {
  ...
  it('should set error when called without parameters', function() {
    ...
    expect(semaphore.__noop)
			.to.have.been.calledWithExactly(semaphore.lastError, false);
    expect(semaphore.lastError.message)
			.to.equal('Action parameters is required');
  });
  ...
  it('should set error when called without parameters', function() {
    ...
    expect(semaphore.requestOptions)
			.to.have.been.calledWithExactly('invalid', undefined);
    expect(semaphore.__request).to.not.have.been.called;
    expect(semaphore.__noop).to.have.been.called.once;
    expect(semaphore.__noop)
			.to.have.been.calledWithExactly(semaphore.lastError, false);
    expect(semaphore.lastError.message)
			.to.equal('Invalid Semaphore API action');
  });
  ...
  it('should call .__request() when called with valid action', function() {
    ...
    expect(semaphore.requestOptions).to.have.been.called.once;
    expect(semaphore.requestOptions)
			.to.have.been.calledWithExactly('account', undefined);

    expect(semaphore.__request).to.have.been.called.once;
    expect(semaphore.__request.firstCall.args[0]).to.be.an('object');
    expect(semaphore.__request.firstCall.args[0])
			.to.deep.equal(semaphore.requestOptions.returnValues[0]);
  ...
  it('should call .__request() when called with action/callback', function() {
    var err = null,
      res = { data: 'response' },
      body = { data: 'body' };
